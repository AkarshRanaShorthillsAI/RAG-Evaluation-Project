...
#     -  Important Point - "If no job Listing is there in required experience range then don't print anything".
...
def log_interaction(query, retrieved_jobs, ai_response):
    """Logs user queries, retrieved job results, and AI-generated responses in a daily log file."""
    # Get current date to create daily log file
    current_date = datetime.now().strftime("%Y-%m-%d")
    daily_log_file = os.path.join(BASE_DIR, "Logs", f"{current_date}.log")
    ...
    try:
        with open(daily_log_file, "a", encoding="utf-8") as log_file:
            log_file.write(json.dumps(log_entry, indent=4) + "\n")
        print(f"✅ Interaction logged to {daily_log_file}!")
    except Exception as e:
